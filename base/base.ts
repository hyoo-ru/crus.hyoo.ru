namespace $ {
	export class $hyoo_crowds_base extends $hyoo_crowds_dict {
		...
		selection( next?: readonly( readonly[ number, number ] )[] ) {
			return ( this.dive( 'selection', $hyoo_crowds_reg ).value_vary( next ) ?? [ [ 0, 0 ], [ 0, 0 ] ] ) as readonly( readonly[ number, number ])[]
		}
